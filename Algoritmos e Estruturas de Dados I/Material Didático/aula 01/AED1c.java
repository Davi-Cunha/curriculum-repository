...
/**
  ...
  */
   public AED1c ( int x, int y, int direction, int beepers )
   {
...
/** para ser executavel devera' haver uma acao principal (main) como ponto de partida. */
   ...
   // saltar linha (forma alternativa)
     ...
     // Robot JK = new Robot ( 1, 1, World.EAST, 1 ); // uso de construtor (new) para criar
        ...
        AED1c JK = new AED1c ( 1, 1, World.EAST, 1 ); // uso de construtor (new) para criar
        ...
        do                           // <- inicio de bloco de repeticao
        {
          if ( JK.nextToABeeper( ) )
          {
             JK.pickBeeper( );
        ...
        while ( JK.beepers( ) == 0 );// <- repetir enquanto nao pegar marcador

        JK.turnLeft  ( );
        JK.turnLeft  ( );
        JK.turnOff( );

     // OBS.: A tarefa termina sem estar o robo na posicao e na direcao iniciais!
