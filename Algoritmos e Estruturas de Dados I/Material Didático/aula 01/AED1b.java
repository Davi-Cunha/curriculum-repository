...
/*
   ...
   Se os arquivos IO.jar e jkarel.jar estiverem na mesma pasta, usar:
   para compilar: javac -cp IO.jar;jkarel.jar;./ AED1.java
   para executar: java  -cp IO.jar;jkarel.jar;./ AED1
*/
...
*/
public class AED1b
       extends Robot                            // <- ampliar definicoes
...
/**
    Construtor para objeto a ser criado segundo esta classe.
  */
   public AED1b ( int x, int y, int direction, int beepers )
   {
    // repassar dados para o construtor da classe Robot
...
/** para ser executavel devera' haver uma acao principal (main) como ponto de partida. */
   ...
   // saltar linha (forma alternativa)
     ...
     // String  - armazenador para simbolos
        ...
        String anEmptyString = "";   // dado vazio      , tamanho = 0
     ...
     // Robot JK = new Robot ( 1, 1, World.EAST, 1 ); // uso de construtor (new) para criar
        ...
        AED1b JK = new AED1b ( 1, 1, World.EAST, 1 ); // uso de construtor (new) para criar

        IO.pause( );

        JK.move ( );         // <-  objeto (JK) usara' o metodo move() da classe (Robot)

        if ( JK.hasBeepers( ) ) //  testar se tem marcador
        {
           JK.putBeeper ( ); // <-  colocar marcador, se tiver algum
        } // fim se

        JK.move ( );
        JK.turnLeft  ( );    // <-  virar 'a esquerda
        JK.move ( );
        JK.turnLeft  ( );
        JK.move ( );
        JK.turnLeft  ( );
        ...
        if ( JK.nextToABeeper( ) )  // testar se ha' marcador no mundo
        {
           JK.pickBeeper( ); // <-  pegar marcador, se tiver algum
        }

        JK.turnRight ( );    // <-  substituicao
